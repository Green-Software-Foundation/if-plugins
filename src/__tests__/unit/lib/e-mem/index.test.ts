...
describe('lib/e-mem: ', () => {
    ...
    describe('configure(): ', () => {
        it('configure returns name and empty config', async () => {
            ...
            await expect(eMemModel.configure({})).resolves.toBeInstanceOf(
                EMemModel
            );
    ...
    describe('execute(): ', () => {
        ...
        it('throws error for missing mem-util input field.', async () => {
            ...
            try {
                ...
                if (error instanceof Error) {
                    expect(error).toBeInstanceOf(Error);
                    expect(error.message).toEqual('EMemModel: mem-util is missing or invalid.');
                }
            }
        });
        it('throws error for missing mem-alloc input field.', async () => {
            ...
            try {
                ...
                if (error instanceof Error) {
                    expect(error).toBeInstanceOf(Error);
                    expect(error.message).toEqual('EMemModel: mem-alloc is missing or invalid.');
                }
            }
        });
        it('does not throw error for missing coefficient, but uses default 0.38.', async () => {
            const eMemModel = new EMemModel();
            await expect(eMemModel.execute([
                {
                    timestamp: '2023-11-02T10:35:31.820Z',
                    duration: 3600,
                    'mem-util': 30,
